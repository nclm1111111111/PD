...
TABLE_STATEMENTS = [
    ...
    order_id VARCHAR(50) NOT NULL UNIQUE COMMENT '报单编号（格式：BD+日期+8位随机）',
    user_id BIGINT NOT NULL COMMENT '用户ID（关联pd_users.id）',  -- 这里从 INT 改为 BIGINT
    order_date DATETIME NOT NULL COMMENT '报单日期',
    delivery_smelter VARCHAR(100) COMMENT '送货冶炼厂',
    driver_name VARCHAR(50) COMMENT '司机姓名',
    driver_id_card VARCHAR(30) COMMENT '身份证号',
    driver_phone VARCHAR(20) COMMENT '司机电话',
    category VARCHAR(50) COMMENT '品类（电动/黑皮/新能源/通信/摩托车/大白）',
    has_weighbill VARCHAR(20) COMMENT '是否自带联单（有联单已发/有联单未发/无联单）',
    license_plate VARCHAR(20) COMMENT '车牌号',
    weighbill_status VARCHAR(20) DEFAULT '待处理' COMMENT '磅单状态：待处理/已有联单',
    is_completed TINYINT(1) DEFAULT 0 COMMENT '是否完成',
    chat_history TEXT COMMENT '对话记录（JSON格式）',
    ...
    INDEX idx_status (weighbill_status),
    FOREIGN KEY (user_id) REFERENCES pd_users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='报单表';
""",
    """
    ...
    # ========== 3. 依赖于 orders 或 pd_deliveries 的表 ==========
    """
    CREATE TABLE IF NOT EXISTS pd_weighbills (
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        order_id BIGINT COMMENT '关联的报单ID（关联orders.id）',
        ...
        net_weight DECIMAL(12, 3) NOT NULL COMMENT '净重（吨）',
        unit_price DECIMAL(12, 2) COMMENT '合同单价',
        total_amount DECIMAL(14, 2) COMMENT '总价（净重×单价）',
        warehouse VARCHAR(100) COMMENT '送货库房',
        target_factory_name VARCHAR(100) COMMENT '目标工厂',
        driver_name VARCHAR(50) COMMENT '司机姓名',
        driver_phone VARCHAR(20) COMMENT '司机电话',
        driver_id_card VARCHAR(30) COMMENT '身份证号',
