...
class WechatPhoneReq(BaseModel):
...
@router.post("/wechat/login", summary="微信小程序登录", response_model=WechatLoginResp)
def wechat_login(body: WechatLoginReq):
    """
    微信小程序登录
    ...
    """
    try:
        result = AuthService.wechat_login(body.code, body.user_info)
        ...
        logger.info(f"微信登录成功: user_id={result['user_id']}, is_new_user={result['is_new_user']}")
        ...
        logger.exception("微信登录异常")
        raise HTTPException(status_code=500, detail="登录失败")


@router.post("/wechat/phone", summary="绑定微信手机号")
    ...
    try:
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT session_key FROM pd_users WHERE id = %s",
                    (current_user["id"],)
                )
                row = cur.fetchone()
                if not row or not row["session_key"]:
                    raise HTTPException(status_code=400, detail="无效的session_key")

                session_key = row["session_key"]

        # 解密手机号
        phone_number = WechatService.get_phone_number(
            session_key,
            body.encrypted_data,
            body.iv
        )
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT id FROM pd_users WHERE phone = %s AND id != %s",
                ...
                # 绑定手机号
                cur.execute(
                    "UPDATE pd_users SET phone = %s, is_verified = 1 WHERE id = %s",
                    (phone_number, current_user["id"])
                )
                conn.commit()
...
@router.post("/auth/refresh", summary="刷新Token", response_model=RefreshTokenResp)
...
@router.get("/users/{user_id}", summary="用户详情", response_model=UserResp)
def get_user(
        user_id: int,
        current_user: dict = Depends(get_current_user)
):
